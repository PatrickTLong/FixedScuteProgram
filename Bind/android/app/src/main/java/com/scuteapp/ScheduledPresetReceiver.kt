...
class ScheduledPresetReceiver : BroadcastReceiver() {
    ...
    private fun showRecurringNotification(context: Context, presetName: String, preset: JSONObject) {
        ...
        try {
            ...
            val notification = NotificationCompat.Builder(context, ALERT_CHANNEL_ID)
                ...
                .setSmallIcon(R.drawable.ic_notification_calendar_check)
    ...
    private fun activateScheduledPreset(context: Context, presetId: String) {
        try {
            ...
            val customBlockedTextColor = targetPreset.optString("customBlockedTextColor", "")
            val customOverlayImage = targetPreset.optString("customOverlayImage", "")
            val customOverlayImageSize = targetPreset.optInt("customOverlayImageSize", 120)
            val customOverlayBgColor = targetPreset.optString("customOverlayBgColor", "")
            val customDismissText = targetPreset.optString("customDismissText", "")
            val customDismissColor = targetPreset.optString("customDismissColor", "")
            val iconPosX = targetPreset.optDouble("iconPosX", 50.0).toFloat()
            val iconPosY = targetPreset.optDouble("iconPosY", 50.0).toFloat()
            val blockedTextPosX = targetPreset.optDouble("blockedTextPosX", 50.0).toFloat()
            val blockedTextPosY = targetPreset.optDouble("blockedTextPosY", 50.0).toFloat()
            val dismissTextPosX = targetPreset.optDouble("dismissTextPosX", 50.0).toFloat()
            val dismissTextPosY = targetPreset.optDouble("dismissTextPosY", 50.0).toFloat()
            val iconVisible = targetPreset.optBoolean("iconVisible", true)
            val blockedTextVisible = targetPreset.optBoolean("blockedTextVisible", true)
            val dismissTextVisible = targetPreset.optBoolean("dismissTextVisible", true)
            val blockedTextSize = targetPreset.optDouble("blockedTextSize", 22.0).toFloat()
            val dismissTextSize = targetPreset.optDouble("dismissTextSize", 10.0).toFloat()
            ...
            Log.d(TAG, "[ACTIVATE] [OVERLAY] bgColor='$customOverlayBgColor', dismissText='$customDismissText', dismissColor='$customDismissColor', image='$customOverlayImage', imageSize=$customOverlayImageSize")
            Log.d(TAG, "[ACTIVATE] [OVERLAY] iconPos=($iconPosX,$iconPosY), blockedTextPos=($blockedTextPosX,$blockedTextPosY), dismissTextPos=($dismissTextPosX,$dismissTextPosY)")
            Log.d(TAG, "[ACTIVATE] [OVERLAY] iconVisible=$iconVisible, blockedTextVisible=$blockedTextVisible, dismissTextVisible=$dismissTextVisible, blockedTextSize=$blockedTextSize, dismissTextSize=$dismissTextSize")
    ...
    private fun showActivationNotification(context: Context, presetName: String, presetId: String) {
        try {
            ...
            val notification = NotificationCompat.Builder(context, ALERT_CHANNEL_ID)
                ...
                .setSmallIcon(R.drawable.ic_notification_alarm_exclamation)
                .setPriority(NotificationCompat.PRIORITY_HIGH)
                .setCategory(NotificationCompat.CATEGORY_ALARM)
                .setAutoCancel(true)
                .setContentIntent(pendingIntent)
