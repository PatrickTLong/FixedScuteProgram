...
class ScheduleModule(reactContext: ReactApplicationContext) :
    ...
    /**
     * Get the scheduled presets JSON from native SharedPreferences.
     * This is useful when native code has updated the preset dates (e.g., recurring preset renewal)
     * and the React Native side needs to sync these changes to the backend.
     */
    @ReactMethod
    fun getScheduledPresetsFromNative(promise: Promise) {
        try {
            ...
            Log.d(TAG, "getScheduledPresetsFromNative: ${presetsJson?.take(200) ?: "null"}")
            promise.resolve(presetsJson)
        } catch (e: Exception) {
            Log.e(TAG, "Error getting scheduled presets from native", e)
            promise.reject("ERROR", "Failed to get scheduled presets: ${e.message}")
        }
    }

    /**
     * Check if the app was launched from a recurring preset pause notification.
     * Returns data if so, including any renewed preset info.
     */
    @ReactMethod
    fun getRecurringPresetPauseLaunchData(promise: Promise) {
        try {
            val prefs = reactApplicationContext.getSharedPreferences(LAUNCH_PREFS, Context.MODE_PRIVATE)
            val wasLaunchedFromRecurring = prefs.getBoolean("recurring_preset_paused", false)

            if (!wasLaunchedFromRecurring) {
                promise.resolve(null)
                return
            }

            // Check if launch was recent (within last 30 seconds) to avoid stale data
            val launchTime = prefs.getLong("recurring_launch_time", 0)
            val now = System.currentTimeMillis()
            if (now - launchTime > 30000) {
                // Launch data is stale, clear it
                clearRecurringPauseLaunchDataInternal()
                promise.resolve(null)
                return
            }
            ...
            Log.e(TAG, "Error getting recurring preset pause launch data", e)
            promise.reject("ERROR", "Failed to get recurring pause launch data: ${e.message}")
        }
    }

    /**
     * Clear the recurring preset pause launch data (call after handling the launch)
     */
    @ReactMethod
    fun clearRecurringPauseLaunchData(promise: Promise) {
        try {
            clearRecurringPauseLaunchDataInternal()
            promise.resolve(true)
        } catch (e: Exception) {
            Log.e(TAG, "Error clearing recurring pause launch data", e)
            promise.reject("ERROR", "Failed to clear recurring pause launch data: ${e.message}")
        }
    }

    private fun clearRecurringPauseLaunchDataInternal() {
        val prefs = reactApplicationContext.getSharedPreferences(LAUNCH_PREFS, Context.MODE_PRIVATE)
        prefs.edit()
            .remove("recurring_preset_paused")
            .remove("recurring_launch_time")
            .apply()
        Log.d(TAG, "Cleared recurring pause launch data")
    }
